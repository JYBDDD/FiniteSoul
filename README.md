...
12. 상점 및 인벤토리 구현, 인벤토리 저장데이터 추가
	-> 상점 + 인벤토리 데이터 및 설정
		-> 인벤토리
			-> SlotGenerator 클래스에서 인벤토리 스크롤뷰 자식 Content에 인벤토리 슬롯 생성
			-> 
...
1. 할 것    TODO
                         ...
                         4. 인벤토리
				-> 현재 인벤토리 버튼이 없으면 스크롤뷰 드래그가 안되는 버그가 있음 (일단 버튼 하나 대충만들어서 쓰고있음 - 강사님한테 물어보기)
				
				-> 인벤토리 슬롯 인벤토리에 slotGenerator로 채워주는 부분까지 했음   TODO
				-> 상점 아이템 만들어서 넣기
				-> 마우스가 데이터가 있는 슬롯을 가리킬경우, 인벤토리 위 설명창에 설명 띄우기
				-> 위에꺼 다되면 밑에꺼 보면서 
