...
1. 할 것    TODO
			 ...
			 6. NPC(상점) 에게 말을 걸때 G키로 상호작용을 할수있도록 만들기
				...
				-> 인벤토리 데이터는 모닥불이나 플레이어가 사망하였다면 데이터를 저장하도록 한다 
					-> 저장된 인벤토리 데이터 불러오는것은 설정이 완료된것 같음(확인 해야함)
					-> 데이터 저장, 데이터 초기화 설정 완료(확인 해야함)
	

	
				-> 아이템 구매,판매창 만들기]
						-> 인벤토리에 아이템 설정 셋팅중 TODO
						-> SellPurchaseWindow 에서 ONCilck 함수 조정중임 TODO  @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
				...
				-> Z 키 클릭시 물약이 마나 <-> 체력 물약으로 변경가능하도록
					-> 소지품 물약의 개수는 본래개수의 -1로 표시되도록 해야함
