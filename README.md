...
14. 사운드 추가 및 설정
	-> 사운드 매니저생성
	-> 딕셔너리로 모든 사운드를 Add 시키고 필요한 사운드는 Contains로 단일, 멀티(여러개 사운드 랜덤) 소리 설정이 가능하도록 하였음
		-> 셋팅 윈도우 생성
			-> 해당 윈도우에서 BGM, Effect 소리 설정 가능
			-> StartScene을 제외한 씬에서는 시작씬으로 되돌아가는 버튼 생성
				-> 시작씬으로 돌아갈시 현재 플레이어 위치값을 제외한 모든 데이터 저장
	
	/ 참고사항 /
	// 씬 전환을 시킬시 빌드 0번이 아닌 다른씬으로의 전환은 호출의 문제가 없지만 0번씬으로 호출시 씬의 시작점으로 인식되어
		 Start,Awake가 재호출이 된다 
		-> 이게 보통은 문제가 없지만 DontDestroyOnLoad 와 함께 호출될경우 전에 파괴되지 않도록 설정한 값이 존재하는 상태에서
			- 0번에서 다시 초기상태의 설정한 값을 DontDestroyOnLoad 안으로 넣으면서 호출하는 다른값(GetInstanceId 체크해보면 달라졌을것)이 존재할경우
			- 새로 설정된값이 존재하는 값을 밀어내면서 발생하는 오류가 발생할수 있음 
				(값이 프로젝트상에서 이미들어있는 값을 다시 새값으로 넣으려해서 인스턴스 ID가 달라져서 그럼)
			- Ex) DrawStageWindow Start() 에서 작성중
	
15. 빌드시 오류사항 수정
	-> NowtonsoftJson 으로 데이터를 불러올시 PlatformNotSupportedException (이 플랫폼에서 작업이 지원되지 않습니다) 오류 출력시
		- 프로젝트 설정 편집 > 플레이어 > > 기타 설정 > API 호환성 수준 : .NET 2.0 -> .NET 4.x 으로 변경할 경우 잘 실행됨
	-> 스탯 상승후 몬스터에게 사망시 레벨업 데이터 미적용 버그 수정
