...
11. 상점 및 인벤토리 구현, 인벤토리 저장데이터 추가, 궁수(발사체) 설정, 파티클(이펙트) 추가
...
1. 할 것    TODO
			...
			1. 플레이어의 체력이 0일시 죽도록 하는거
				-> 죽었다면 천천히 페이드아웃 시켜주고, 해당씬으로 재로드 한다  TODO
	...
	4.10 해야되는데 아직 구현 안한것
		1. 몬스터 아이템 드랍
		2. SerilizeField 로 만든것 설명은 ToolTip으로 넣어주기
		3. 애니메이션으로 몬스터 죽었을때 콜라이더 껏는데, 나중에 플레이어 죽은후 재로드 하였을때 잘 맞는지 확인하기
