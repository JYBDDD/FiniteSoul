...
1. 할 것    TODO
			 ...
			 6. NPC(상점) 에게 말을 걸때 G키로 상호작용을 할수있도록 만들기
				...
				-> 인벤토리 데이터는 모닥불이나 플레이어가 사망하였다면 데이터를 저장하도록 한다 
					-> 저장된 인벤토리 데이터 불러오는것은 설정이 완료된것 같음(확인은 해야함)
					-> 데이터 저장하는 구문 ResourceUtil의 플레이어 데이터 저장하는 곳에 설정할것 TODO
	...
	- 몬스터의 아이템 드랍은 해당 몬스터의 아이템 드랍 확률안에 들어갔을 시, 아이템드랍확률를 확률을 점점 줄이는 방식으로 계산하여 드랍할것임
				...
				-> SellPurChaseWindow 부터 만들면 됨
				-> 아이템 구매,판매창 만들기]
					-> InputField로 아이템 판매 개수 작성가능 (자기가 보유한 아이템 갯수 이상일시 자동으로 자신이 보유한 아이템 갯수로 변경)
					-> InputField로 아이템 구매 개수 작성가능 (아이템금액 * 구매할 아이템 금액이 자신이 보유한 룬을 초과할 경우
											 자신이 구매가능한 최대개수로 변경)
						-> 상점 + 인벤창 종료시 구매,판매창 재설정 필요
	
				-> 상점 슬롯을 클릭했을시 아이템 구매창이 출력
				-> 인벤토리 슬롯을 클릭했을시 아이템 판매창이 출력
	
				
				-> 아이템 판매시 자신의 인벤토리 아이템 데이터 재설정
				-> 아이템 구매시 자신의 인벤토리 아이템 데이터 재설정 (인벤토리 리스트에서 아이템데이터가 비어있는 곳에 삽입)
	
	
	
				 ★ 참고사항 - 아이템 데이터가 비어있는 곳을 for문으로 넣으려고 할경우 오류가 발생할수 있음 확인바람 !!!!!!!!!!!!!!!!
