...
14. 사운드 추가 및 설정
	...
	// 씬 전환을 시킬시 빌드 0번이 아닌 다른씬으로의 전환은 호출의 문제가 없지만 0번씬으로 호출시 씬의 시작점으로 인식되어
		...
		-> 이게 보통은 문제가 없지만 DontDestroyOnLoad 와 함께 호출될경우 전에 파괴되지 않도록 설정한 값이 존재하는 상태에서
			- 0번에서 다시 초기상태의 설정한 값을 DontDestroyOnLoad 안으로 넣으면서 Start,Awake에서 호출하는 다른값이 존재할경우
			- 새로 설정된값이 존재하는 값을 밀어내면서 발생하는 오류가 발생할수 있음 (정적 변수로 본인 스크립트를 참조할경우..? 라고하는게 맞을듯)
			- Ex) DrawStageWindow Start() 에서 작성중
22.05.02 ~ 22.05.03
	
15. 빌드시 오류사항 수정
	-> NowtonsoftJson 으로 데이터를 불러올시 PlatformNotSupportedException (이 플랫폼에서 작업이 지원되지 않습니다) 오류 출력시
		- 프로젝트 설정 편집 > 플레이어 > > 기타 설정 > API 호환성 수준 : .NET 2.0 -> .NET 4.x 으로 변경할 경우 잘 실행됨 (원리는 )
